...
def main():
    ...
    for url, date_text in reports:
        ...
        try:
            ...
            image_bytes, description = extract_section_5(pdf_bytes)
            
            if image_bytes:
                # Save results
                filename_base = slugify(url)
                save_output(image_bytes, description, filename_base)
    ...
    if newly_processed > 0:
        logging.info(f"Successfully processed {newly_processed} new reports. Saving state.")
        save_state(state)
    else:
